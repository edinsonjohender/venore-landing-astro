...
export function AuthPage() {
  ...
  const autoProvider = params.get("provider") as Provider | null;
  ...
  const handleLogin = async (provider: Provider) => {
    ...
    // Build callback URL — pass redirect_uri through so callback page knows where to send tokens
  ...
  // Auto-start OAuth when `provider` query param is present (desktop app flow)
  useEffect(() => {
    if (autoProvider && (autoProvider === "github" || autoProvider === "google")) {
      handleLogin(autoProvider);
    }
    ...
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <div className="border border-border rounded-lg p-6 bg-background-secondary">
          ...
          {error && (
            <p className="mt-4 text-sm text-status-critical text-center">
              {error}
            </p>
          )}
